...
describe('Testes dos Módulos Usuário e Auth (e2e)', () => {  // descrição do teste e2e
  ...
  // testes
  it("01 - Deve criar um usuário", async () => { // descrição do teste
    const resposta = await request(app.getHttpServer()) // faz a requisição para o servidor nest
    .post('/usuarios/cadastrar').send({ // envia os dados do usuário para criar
      nome: "root",
      usuario: "root@root.com",
      senha: "12345678",
      foto: "-"
    }).expect(201) // espera a resposta com status code 201; 
